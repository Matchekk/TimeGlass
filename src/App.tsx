...
function applyLeaveToSummary(
	...
	if (
		settings.defaultPaidAbsenceBehavior === "counts_as_target" &&
...
function activeSessionMinutes(
	...
	return Math.max(
		0,
		Math.round(
			(now.getTime() - new Date(activeEntry.start_time).getTime()) / 60_000,
		),
	);
}
...
export function App() {
	...
	const hasActiveSession = useMemo(
		() => data?.entries.some((entry) => !entry.end_time) ?? false,
		[data?.entries],
	);

	useEffect(() => {
		const interval = getRefreshIntervalMs(
			data?.settings.lowRamMode ?? true,
			isVisible,
			hasActiveSession,
		);
		const timer = window.setInterval(() => setTick(new Date()), interval);
		return () => window.clearInterval(timer);
	}, [data?.settings.lowRamMode, hasActiveSession, isVisible]);
	...
	const refresh = useCallback(async () => {
		try {
			...
			const allKeys = Array.from(
				new Set([...weekKeys, ...monthKeys, ...yearKeys, todayKey]),
			);
			const [
				settings,
				entries,
			...
			const allRecordedKeys = Array.from(
				new Set([
					...
					...leaveEntries.flatMap((entry) =>
						eachDateInRange(entry.start_date, entry.end_date),
					),
				]),
			).sort();
			...
			setData({
				...
				overrides,
				leaveEntries,
				flexBalanceMinutes: calculateFlexBalance(
					allDays,
	...
	const liveData = useMemo(() => {
		...
		const todayOverride = data.overrides.find(
			(override) => override.date === data.today.date,
		);
		const summary = summarizeDay(
			data.today.date,
			data.entries,
		...
		const today = applyLeaveToSummary(
			summary,
			findLeaveForDate(data.leaveEntries, data.today.date),
			data.settings,
	...
	useEffect(() => {
		if (!liveData) return;
		// Native Aufrufe nur ausloesen, wenn sich der Wert wirklich geaendert hat. Der
		// Tick laeuft im Sekundentakt, die Tray-Texte sind aber minutengenau – so sparen
		// wir ~59 ueberfluessige Aufrufe pro Minute.
		if (lastCloseToTray.current !== liveData.settings.closeToTray) {
			lastCloseToTray.current = liveData.settings.closeToTray;
			void invoke("set_close_to_tray", {
				enabled: liveData.settings.closeToTray,
			});
		}
		const activeEntry =
		...
		const payload = {
			...
			session: activeEntry
				? `Aktuelle Session: ${formatMinutes(activeSessionMinutes(activeEntry, tick))}`
				: "Aktuelle Session: -",
			toggleLabel: activeEntry ? "Ausstempeln" : "Einstempeln",
		};
		const serialized = JSON.stringify(payload);
		if (lastTrayPayload.current !== serialized) {
			lastTrayPayload.current = serialized;
			void invoke("update_tray_status", payload).catch(() => undefined);
		}
	}, [liveData, tick]);
	...
	useEffect(() => {
		...
		const decisions = getReminderDecisions(
			liveData.settings,
			liveData.today,
			activeEntry,
			state,
	...
	useEffect(() => {
		if (!liveData || minimizedOnce.current || !liveData.settings.startMinimized)
			return;
	...
	const todayRemaining =
		liveData && liveData.today.targetMinutes != null
			...
			<aside className="sidebar glass-panel">
				...
				{weekTotal && weekTotal.differenceMinutes != null && (
					<div className="sidebar-meta">
						...
						<strong>
							{weekTotal.differenceMinutes >= 0 ? "+" : ""}
							{Math.round((weekTotal.differenceMinutes / 60) * 10) / 10} h
						</strong>
					</div>
				)}
			</aside>
