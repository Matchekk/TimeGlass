...
describe("time calculations", () => {
	it("calculates day net with one session", () => {
		const summary = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", "2026-05-20T16:00:00.000Z")],
			undefined,
			defaultSettings,
			now,
		);
	...
	it("calculates positive day difference", () => {
		const summary = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", "2026-05-20T17:30:00.000Z")],
			undefined,
			defaultSettings,
			now,
		);
	...
	it("calculates negative day difference", () => {
		const summary = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", "2026-05-20T15:00:00.000Z")],
			undefined,
			defaultSettings,
			now,
		);
	...
	it("keeps free days at target 0 even when workdays include the date", () => {
		...
		expect(
			summarizeDay(
				date,
				[entry("2026-05-20T08:00:00.000Z", "2026-05-20T09:00:00.000Z")],
				override,
				defaultSettings,
				now,
			).targetMinutes,
	...
	it("adds start flex balance correctly", () => {
		const day = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", "2026-05-20T17:00:00.000Z")],
			undefined,
			defaultSettings,
			now,
		);
	...
	it("includes active running session for live display", () => {
		const summary = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", null)],
			undefined,
			defaultSettings,
			now,
		);
	...
	it("excludes the running day (active session) from the flex balance", () => {
		const closed = summarizeDay(
			"2026-05-19",
			[entry("2026-05-19T08:00:00.000Z", "2026-05-19T17:00:00.000Z")],
			undefined,
			defaultSettings,
			new Date("2026-05-19T18:00:00.000Z"),
		);
		const activeToday = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", null)],
			undefined,
			defaultSettings,
		...
		expect(activeToday.hasActiveSession).toBe(true);
		// Nur der abgeschlossene Vortag (+60) zaehlt, der laufende Tag wird ignoriert.
		expect(calculateFlexBalance([closed, activeToday], 0)).toBe(60);
	});

	it("folds the day into the balance once it is closed (clock-out)", () => {
		const closedToday = summarizeDay(
			date,
			[entry("2026-05-20T08:00:00.000Z", "2026-05-20T15:00:00.000Z")],
			undefined,
			defaultSettings,
		...
		expect(closedToday.hasActiveSession).toBe(false);
		// 7h gearbeitet bei 8h Soll -> -60 fliesst nach dem Ausstempeln ins Konto.
		expect(calculateFlexBalance([closedToday], 0)).toBe(-60);
	});
	...
	it("detects long active sessions", () => {
		expect(
			isLongActiveSession(entry("2026-05-20T01:00:00.000Z", null), 600, now),
		).toBe(true);
	});
	...
	it("calculates leave time for zero balance and desired plus", () => {
		...
		expect(estimate.leaveAtZero?.toISOString()).toBe(
			"2026-05-20T16:00:00.000Z",
		);
		expect(estimate.leaveAtDesiredPlus?.toISOString()).toBe(
	...
	it("marks daily target as reached", () => {
		...
		expect(
			calculateLeaveTimeEstimate(summary, null, 30, now).targetReached,
		).toBe(true);
	});
});
...
function dayEntry(
	...
	return entry(
		`${dateKey}T${pad(startHour)}:00:00.000Z`,
		`${dateKey}T${pad(endHour)}:00:00.000Z`,
	);
}
...
describe("work model: fixed_weekly_distributed", () => {
	...
	it("derives 8h target on each active day", () => {
		expect(getTargetMinutesForDate("2026-05-18", undefined, settings)).toBe(
			480,
		);
		expect(getTargetMinutesForDate("2026-05-20", undefined, settings)).toBe(
			480,
		);
		expect(getTargetMinutesForDate("2026-05-22", undefined, settings)).toBe(
			480,
		);
	});
	it("0 target on inactive workday (Tuesday)", () => {
		...
		const summary = summarizeDay(
			...
			undefined,
			settings,
			new Date(`${tuesday}T18:00:00.000Z`),
		);
		expect(summary.differenceMinutes).toBe(0);
	});
	...
	it("respects 20h spread over 4 days = 5h each", () => {
		...
		expect(getTargetMinutesForDate("2026-05-18", undefined, fourDayWeek)).toBe(
			300,
		);
	});
});
...
describe("work model: custom_weekday_targets", () => {
	...
	it("uses Tuesday target 6h", () => {
		expect(getTargetMinutesForDate("2026-05-19", undefined, settings)).toBe(
			360,
		);
	});
...
describe("work model: variable_weekly_target", () => {
	...
	it("week with 18h is -2h weekly balance", () => {
		...
		const summaries = weekDays.map((key) =>
			summarizeDay(
				key,
				[],
				undefined,
				settings,
				new Date(`${key}T18:00:00.000Z`),
			),
		);
		// simulate net = 18h by injecting netMinutes via override-equivalent entries: just construct
		const synthetic = summaries.map((s, idx) => ({
			...s,
			netMinutes: idx === 0 ? 9 * 60 : idx === 1 ? 9 * 60 : 0,
		}));
	...
	it("week with 22h is +2h weekly balance", () => {
		...
		const synthetic = summaries.map((s, idx) => ({
			...s,
			netMinutes: idx === 0 ? 11 * 60 : idx === 1 ? 11 * 60 : 0,
		}));
		const week = summarizePeriod(synthetic, { settings, kind: "week" });
		expect(week.differenceMinutes).toBe(2 * 60);
	});
...
describe("work model: no_target_tracking", () => {
	...
	it("flex balance is null (not active)", () => {
		...
		expect(
			calculateFlexBalance(summaries, 0, { settings, kind: "year" }),
		).toBeNull();
	});
...
describe("leave/absence behavior across work models", () => {
	...
	it("vacation on a target-0 day produces no extra hours", () => {
		...
		const summary = summarizeDay(
			...
			override,
			settings,
			new Date(`${saturday}T18:00:00.000Z`),
		);
		expect(summary.targetMinutes).toBe(0);
		expect(summary.differenceMinutes).toBe(0);
	});
...
describe("calculateTodayExitOptions", () => {
	...
	it("computes exit times when tracking", () => {
		...
		expect(out.exitAtZeroBalance?.toISOString()).toBe(
			"2026-05-20T14:00:00.000Z",
		);
		expect(out.exitAtDesiredBalance?.toISOString()).toBe(
