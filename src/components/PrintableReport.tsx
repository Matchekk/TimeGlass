...
export function PrintableReport({
  ...
  const rows = days.filter(
    (day) => day.netMinutes > 0 || (day.targetMinutes ?? 0) > 0 || day.dayType !== "work",
  );
  ...
  return (
    <div className="print-report" aria-hidden="true">
      <div className="print-header">
        ...
        <h2>{title}</h2>
        <p>Erstellt am {new Intl.DateTimeFormat("de-DE", { dateStyle: "long", timeStyle: "short" }).format(new Date())}</p>
        <p className="print-disclaimer">Private Übersicht – keine rechtsverbindliche Zeiterfassung.</p>
      </div>
        ...
        <tbody>
          {rows.map((day) => (
            <tr key={day.date}>
              ...
              {showTarget && <td>{day.differenceMinutes != null ? formatMinutes(day.differenceMinutes, true) : "—"}</td>}
              <td>{[dayTypeLabels[day.dayType] ?? day.dayType, day.note].filter(Boolean).join(" · ")}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr>
            ...
            {showTarget && <td>{total.differenceMinutes != null ? formatMinutes(total.differenceMinutes, true) : "—"}</td>}
            <td />
          </tr>
        </tfoot>
      </table>
