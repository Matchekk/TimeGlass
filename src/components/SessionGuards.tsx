...
function suggestedClockOutIso(
	activeEntry: TimeEntry,
	settings: Settings,
): string {
	const startDateKey = toDateKey(new Date(activeEntry.start_time));
	const [hours, minutes] = (settings.reminderClockOutTime || "17:00")
		.split(":")
		.map(Number);
	...
	if (suggestion.getTime() <= start.getTime()) {
		return new Date(
			start.getTime() + Math.max(0, settings.standardTargetMinutes) * 60_000,
		).toISOString();
	}
...
export function SessionGuards({
	...
	const [forgotDismissedId, setForgotDismissedId] = useState<number | null>(
		null,
	);
	...
	const sessionMinutes = activeEntry
		...
		: 0;
	const crossesDay = activeEntry
		? toDateKey(new Date(activeEntry.start_time)) < toDateKey(now)
		: false;
	const forgotActive =
		Boolean(activeEntry) &&
		(crossesDay || sessionMinutes >= FORGOT_THRESHOLD_MINUTES);

	useEffect(() => {
		if (activeEntry && lastForgotId.current !== activeEntry.id) {
			lastForgotId.current = activeEntry.id;
			setCorrectTime(
				dateTimeLocalValue(suggestedClockOutIso(activeEntry, settings)),
			);
		}
		if (!activeEntry) {
	...
	async function applyForgotCorrection() {
		...
		try {
			...
			if (
				new Date(endIso).getTime() <= new Date(activeEntry.start_time).getTime()
			) {
		...
		void invoke<number>("get_idle_seconds")
			.then((seconds) => {
				...
				const thresholdSeconds =
					Math.max(1, settings.idleThresholdMinutes) * 60;
	...
	async function trimIdleTime() {
		...
		try {
			if (
				new Date(idleSinceIso).getTime() <=
				new Date(activeEntry.start_time).getTime()
			) {
				onError(
					"Die Abwesenheit liegt vor dem Einstempeln – bitte manuell korrigieren.",
				);
				return;
			}
			await updateEntry(
			...
			onError(
				err instanceof Error
					? err.message
					: "Abwesenheit konnte nicht abgezogen werden.",
			);
		}
	}
	...
	const showForgot =
		forgotActive && activeEntry != null && forgotDismissedId !== activeEntry.id;
	const showIdle =
		idleMinutes != null && idleSinceIso != null && activeEntry != null;
		...
		<div className="guard-stack">
			{showIdle && (
				<div
					...
					role="alertdialog"
					aria-label="Abwesenheit erkannt"
				>
					<div>
						<strong>Abwesend seit {formatClock(idleSinceIso)}?</strong>
						<span>
							Keine Eingaben seit {formatMinutes(idleMinutes!)} Stunden. Soll
							die Abwesenheit von der laufenden Session abgezogen werden
							(Ausstempeln auf {formatClock(idleSinceIso)})?
						</span>
					</div>
			...
			{showForgot && (
				<div
					className="guard-banner"
					role="alertdialog"
					aria-label="Vergessen auszustempeln"
				>
					<div>
						<strong>
							Noch eingestempelt seit {formatClock(activeEntry!.start_time)}
						</strong>
						<span>
							Die Session läuft seit {formatMinutes(sessionMinutes)} Stunden
							{crossesDay ? " (über Nacht)" : ""}. Hast du vielleicht vergessen
							auszustempeln?
						</span>
					</div>
					<div className="guard-actions">
						...
						<button
							type="button"
							className="secondary-button"
							onClick={() => void applyForgotCorrection()}
						>
