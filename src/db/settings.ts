...
function parseRegion(value: string | undefined): GermanRegion {
	if (value && VALID_REGIONS.has(value as GermanRegion))
		return value as GermanRegion;
...
function parseWorkModelMode(value: string | undefined): WorkModelMode {
	if (value && VALID_WORK_MODES.has(value as WorkModelMode))
		return value as WorkModelMode;
...
function parseWeekdayTargets(value: string | undefined): number[] {
	...
	try {
		...
		if (
			...
			parsed.length === 7 &&
			parsed.every((entry) => Number.isFinite(Number(entry)))
		) {
...
export async function getSettings(): Promise<Settings> {
	...
	return {
		standardTargetMinutes: numberValue(
			values.get(keys.standardTargetMinutes),
			defaultSettings.standardTargetMinutes,
		),
		workdays: JSON.parse(
			values.get(keys.workdays) ?? JSON.stringify(defaultSettings.workdays),
		) as number[],
		trackingStartDate:
			values.get(keys.trackingStartDate) || defaultSettings.trackingStartDate,
		startBalanceMinutes: numberValue(
			values.get(keys.startBalanceMinutes),
			defaultSettings.startBalanceMinutes,
		),
		autoBreakEnabled: boolValue(
			values.get(keys.autoBreakEnabled),
			defaultSettings.autoBreakEnabled,
		),
		autoBreakThresholdMinutes: numberValue(
			values.get(keys.autoBreakThresholdMinutes),
			defaultSettings.autoBreakThresholdMinutes,
		),
		autoBreakMinutes: numberValue(
			values.get(keys.autoBreakMinutes),
			defaultSettings.autoBreakMinutes,
		),
		autostartEnabled: boolValue(
			values.get(keys.autostartEnabled),
			defaultSettings.autostartEnabled,
		),
		startMinimized: boolValue(
			values.get(keys.startMinimized),
			defaultSettings.startMinimized,
		),
		closeToTray: boolValue(
			values.get(keys.closeToTray),
			defaultSettings.closeToTray,
		),
		lowRamMode: boolValue(
			values.get(keys.lowRamMode),
			defaultSettings.lowRamMode,
		),
		reminderLongSessionEnabled: boolValue(
			values.get(keys.reminderLongSessionEnabled),
			defaultSettings.reminderLongSessionEnabled,
		),
		reminderLongSessionMinutes: numberValue(
			values.get(keys.reminderLongSessionMinutes),
			defaultSettings.reminderLongSessionMinutes,
		),
		reminderClockOutEnabled: boolValue(
			values.get(keys.reminderClockOutEnabled),
			defaultSettings.reminderClockOutEnabled,
		),
		reminderClockOutTime:
			values.get(keys.reminderClockOutTime) ??
			defaultSettings.reminderClockOutTime,
		reminderNoTimeTodayEnabled: boolValue(
			values.get(keys.reminderNoTimeTodayEnabled),
			defaultSettings.reminderNoTimeTodayEnabled,
		),
		reminderTargetReachedEnabled: boolValue(
			values.get(keys.reminderTargetReachedEnabled),
			defaultSettings.reminderTargetReachedEnabled,
		),
		unusualSessionMinutes: numberValue(
			values.get(keys.unusualSessionMinutes),
			defaultSettings.unusualSessionMinutes,
		),
		notifyUnusualSession: boolValue(
			values.get(keys.notifyUnusualSession),
			defaultSettings.notifyUnusualSession,
		),
		roundingMode: (values.get(keys.roundingMode) ??
			defaultSettings.roundingMode) as typeof defaultSettings.roundingMode,
		annualVacationDays: numberValue(
			values.get(keys.annualVacationDays),
			defaultSettings.annualVacationDays,
		),
		vacationCarryoverDays: numberValue(
			values.get(keys.vacationCarryoverDays),
			defaultSettings.vacationCarryoverDays,
		),
		vacationYear: numberValue(
			values.get(keys.vacationYear),
			defaultSettings.vacationYear,
		),
		defaultPaidAbsenceBehavior: (values.get(keys.defaultPaidAbsenceBehavior) ??
			defaultSettings.defaultPaidAbsenceBehavior) as typeof defaultSettings.defaultPaidAbsenceBehavior,
		...
		weeklyTargetMinutes: numberValue(
			values.get(keys.weeklyTargetMinutes),
			defaultSettings.weeklyTargetMinutes,
		),
		weekdayTargets: parseWeekdayTargets(values.get(keys.weekdayTargets)),
		showOvertimeBalance: boolValue(
			values.get(keys.showOvertimeBalance),
			defaultSettings.showOvertimeBalance,
		),
		showDailyDelta: boolValue(
			values.get(keys.showDailyDelta),
			defaultSettings.showDailyDelta,
		),
		desiredBalanceMinutes: numberValue(
			values.get(keys.desiredBalanceMinutes),
			defaultSettings.desiredBalanceMinutes,
		),
		globalShortcutEnabled: boolValue(
			values.get(keys.globalShortcutEnabled),
			defaultSettings.globalShortcutEnabled,
		),
		globalShortcutAccelerator:
			values.get(keys.globalShortcutAccelerator) ||
			defaultSettings.globalShortcutAccelerator,
		trayLeftClickAction: parseTrayLeftClick(
			values.get(keys.trayLeftClickAction),
		),
		autoBackupEnabled: boolValue(
			values.get(keys.autoBackupEnabled),
		...
		autoBackupRetention: Math.max(
			...
			Math.round(
				numberValue(
					values.get(keys.autoBackupRetention),
					defaultSettings.autoBackupRetention,
				),
			),
		),
		holidayRegion: parseRegion(values.get(keys.holidayRegion)),
		idleDetectionEnabled: boolValue(
			values.get(keys.idleDetectionEnabled),
			defaultSettings.idleDetectionEnabled,
		),
		idleThresholdMinutes: Math.max(
			1,
			Math.round(
				numberValue(
					values.get(keys.idleThresholdMinutes),
...
export async function saveSettings(settings: Settings): Promise<void> {
	...
	const pairs: Array<[string, string]> = [
		...
		[
			keys.autoBreakThresholdMinutes,
			String(settings.autoBreakThresholdMinutes),
		],
		...
		[
			keys.reminderLongSessionEnabled,
			String(settings.reminderLongSessionEnabled),
		],
		[
			keys.reminderLongSessionMinutes,
			String(settings.reminderLongSessionMinutes),
		],
		...
		[
			keys.reminderNoTimeTodayEnabled,
			String(settings.reminderNoTimeTodayEnabled),
		],
		[
			keys.reminderTargetReachedEnabled,
			String(settings.reminderTargetReachedEnabled),
		],
...
export async function replaceRawSettings(
