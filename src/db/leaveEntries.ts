...
export async function getLeaveEntries(): Promise<LeaveEntry[]> {
	...
	return db.select<LeaveEntry[]>(
		"SELECT * FROM leave_entries ORDER BY start_date, id",
	);
}

export async function getLeaveEntriesForYear(
	year: number,
): Promise<LeaveEntry[]> {
	const db = await getDb();
	return db.select<LeaveEntry[]>(
...
export async function saveLeaveEntry(
	input: Omit<LeaveEntry, "id" | "created_at" | "updated_at"> & {
		id?: number | null;
	},
): Promise<void> {
...
export async function replaceLeaveEntries(
	entries: LeaveEntry[],
): Promise<void> {
	const db = await getDb();
