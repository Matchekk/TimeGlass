...
const workModelOptions: Array<{
	...
	{
		value: "fixed_daily",
		label: "Vollzeit / feste Sollzeit",
		helper: "Feste Sollzeit pro Arbeitstag, z. B. Mo–Fr je 8 Stunden.",
	},
	{
		value: "fixed_weekly_distributed",
		label: "Teilzeit mit festen Tagen",
		helper: "Feste Wochenstunden, verteilt auf gewählte Arbeitstage.",
	},
	{
		value: "custom_weekday_targets",
		label: "Teilzeit mit individuellen Tageszielen",
		helper: "Pro Wochentag eigene Sollzeit, freie Tage sind 0:00.",
	},
	{
		value: "variable_weekly_target",
		label: "Flexible Wochenstunden",
		helper:
			"Feste Wochenstunden, Tagesverteilung ist frei. Keine Tagesdifferenz.",
	},
	{
		value: "no_target_tracking",
		label: "Nur Ist-Zeit",
		helper:
			"Reine Erfassung gearbeiteter Zeit. Keine Über- oder Unterstundenberechnung.",
	},
];
...
export function SettingsPage({
	...
	async function persistSettings(
		...
		if (
			...
			if (!granted) {
				...
				if (permission !== "granted") {
					setMessage(
						"Einstellungen gespeichert. Benachrichtigungen sind nicht erlaubt, Reminder bleiben ohne Systemmeldung.",
					);
				}
			}
		}
		try {
			...
			setMessage(
				"Einstellungen gespeichert. Autostart konnte nicht geändert werden.",
			);
		}
		await saveSettings(settings);
		setMessage((current) =>
			current && current.includes("Autostart") ? current : successMessage,
		);
		await refresh();
	}
	...
	async function exportJson() {
		...
		download(
			`timeglass-${new Date().toISOString().slice(0, 10)}.json`,
	...
	async function exportCsv() {
		...
		const rows = [
			...
			...payload.time_entries.map((entry) =>
				[
					entry.id,
					entry.start_time,
					entry.end_time ?? "",
		...
		download(
			`timeglass-${new Date().toISOString().slice(0, 10)}.csv`,
			rows.join("\n"),
	...
	async function importJson(event: ChangeEvent<HTMLInputElement>) {
		...
		try {
			...
			setMessage(
				"Import abgeschlossen. Die importierten Daten sind jetzt sichtbar.",
			);
			await refresh();
		} catch (err) {
			setMessage(
				err instanceof Error
					...
					: "Import fehlgeschlagen. Bitte eine gültige TimeGlass-JSON-Datei auswählen.",
			);
		} finally {
			event.target.value = "";
		}
	...
	async function runManualBackup() {
		try {
			...
			const path = await invoke<string>("create_db_backup", {
			...
			setMessage(
				err instanceof Error
					? `Backup fehlgeschlagen: ${err.message}`
	...
	async function importHolidays() {
		...
		try {
			const count = await importPublicHolidays(
				germanHolidays(year, settings.holidayRegion),
			);
			setMessage(
				count > 0
			...
			setMessage(
				err instanceof Error
					? `Feiertags-Import fehlgeschlagen: ${err.message}`
					: "Feiertags-Import fehlgeschlagen.",
			);
		}
	}
		...
		<div className="page-stack">
			...
			<section className="glass-panel settings-form">
				...
				<p className="muted wide">
					Bestimmt, wie TimeGlass Sollzeit, Differenzen und Gleitzeit anzeigt.
					Persönliche Übersicht, keine offizielle Lohnabrechnung.
				</p>
				<label className="wide">
					...
					<select
						...
						onChange={(event) =>
							patch({ workModelMode: event.target.value as WorkModelMode })
						}
					>
				...
				<p className="muted wide" id="work-model-helper">
					{
						workModelOptions.find(
							(option) => option.value === settings.workModelMode,
						)?.helper
					}
				</p>
					...
					<>
							...
							>
								{weekdays.map((day) => (
									<button
										type="button"
										className={
											settings.workdays.includes(day.value) ? "active" : ""
										}
										key={day.value}
										aria-pressed={settings.workdays.includes(day.value)}
										onClick={() =>
											patch({
												workdays: settings.workdays.includes(day.value)
													? settings.workdays.filter(
						...
						<label>
							...
							<input
								...
								onChange={(event) =>
									patch({
										standardTargetMinutes:
											parseDurationToMinutes(event.target.value) ??
						...
						<p className="muted wide" id="fixed-daily-help">
							Eingabe als HH:MM, z. B. 8:00.
						</p>
					</>
				)}
				...
				{settings.workModelMode === "fixed_weekly_distributed" && (
					<>
						...
						<p className="muted wide" id="weekly-help">
							Eingabe als HH:MM, z. B. 24:00 für 24 Wochenstunden.
						</p>
						<div className="wide">
				...
				{settings.workModelMode === "custom_weekday_targets" && (
					<div className="wide">
						...
						<p className="muted">
							Tage mit Sollzeit größer 0 zählen automatisch als Arbeitstag.
						</p>
							...
							{weekdayOrder.map((day) => (
								<label key={day.index} className="weekday-target-row">
									...
									<input
										...
										value={formatMinutes(
											settings.weekdayTargets[day.index] ?? 0,
										)}
										onChange={(event) =>
											updateWeekdayTarget(day.index, event.target.value)
										}
									/>
								</label>
							))}
				...
				{settings.workModelMode === "variable_weekly_target" && (
					<>
						...
						<p className="muted wide" id="variable-help">
							In diesem Modus wird keine Tagesdifferenz berechnet. Tage ohne
							Arbeit erzeugen keine Unterstunden.
						</p>
					</>
				)}
				...
				{settings.workModelMode === "no_target_tracking" && (
					<p className="muted wide">
						In diesem Modus erfasst TimeGlass nur gearbeitete Zeit. Überstunden,
						Unterstunden, Gleitzeit und Sollzeit werden nicht berechnet.
					</p>
				)}

				<section
					className="glass-card preview-card wide"
					aria-label="Vorschau Arbeitsmodell"
				>
					...
					<ul className="preview-list">
						<li>
							...
							<strong>
								{previewTodaysTarget == null
									? "Kein Tages-Soll"
									: formatMinutes(previewTodaysTarget)}
							</strong>
						</li>
						<li>
								...
								{settings.workModelMode === "no_target_tracking"
									...
									: settings.workModelMode === "variable_weekly_target"
										...
										: settings.workModelMode === "fixed_weekly_distributed"
											...
											: settings.workModelMode === "custom_weekday_targets"
												? formatMinutes(
														settings.weekdayTargets.reduce(
															(sum, value) =>
				...
				<button
					...
					type="button"
					onClick={() => void persistSettings("Arbeitsmodell gespeichert.")}
				>
					<Save size={16} aria-hidden="true" /> Arbeitsmodell speichern
				</button>
			</section>
			...
			>
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							patch({ trackingStartDate: event.target.value || null })
						}
					/>
				</label>
				<label className="switch-row wide">
					<input
						...
						onChange={(event) =>
							patch({ autoBreakEnabled: event.target.checked })
						}
					/>
				...
				<label>
					Auto-Pause ab
					<input
						value={formatMinutes(settings.autoBreakThresholdMinutes)}
						onChange={(event) =>
							patch({
				...
				<label>
					Auto-Pause Dauer
					<input
						value={formatMinutes(settings.autoBreakMinutes)}
						onChange={(event) =>
							patch({
								autoBreakMinutes:
									parseDurationToMinutes(event.target.value) ??
				...
				<button className="secondary-button wide" type="submit">
					<Save size={16} aria-hidden="true" /> Einstellungen speichern
				</button>
			</form>
			...
			<section className="glass-panel settings-form">
				...
				<label className="switch-row wide">
					<input
						...
						onChange={(event) =>
							patch({ autostartEnabled: event.target.checked })
						}
					/>
				...
				<label className="switch-row wide">
					<input
						...
						onChange={(event) =>
							patch({ startMinimized: event.target.checked })
						}
					/>
				...
				<label className="switch-row wide">
					<input
						...
						onChange={(event) => patch({ lowRamMode: event.target.checked })}
					/>
				...
				<label>
					...
					<select
						...
						onChange={(event) =>
							patch({
								roundingMode: event.target.value as Settings["roundingMode"],
							})
						}
					>
				...
				<button
					className="secondary-button wide"
					type="button"
					onClick={() => void persistSettings("System gespeichert.")}
				>
					<Save size={16} aria-hidden="true" /> System speichern
				</button>
			</section>
			...
			<section className="glass-panel settings-form">
				...
				<label className="switch-row wide">
					<input
						...
						onChange={(event) =>
							patch({ globalShortcutEnabled: event.target.checked })
						}
					/>
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							patch({ globalShortcutAccelerator: event.target.value })
						}
					/>
				</label>
				<p className="muted wide" id="hotkey-help">
					Format z. B. „CmdOrCtrl+Alt+T“ oder „Alt+Shift+S“. Wirkt systemweit,
				...
				<label>
					...
					<select
						...
						onChange={(event) =>
							patch({
								trayLeftClickAction: event.target.value as TrayLeftClickAction,
							})
						}
					>
				...
				<label className="switch-row wide">
					<input
						...
						onChange={(event) =>
							patch({ idleDetectionEnabled: event.target.checked })
						}
					/>
				...
				<label>
					Idle-Schwelle
					<input
						value={formatMinutes(settings.idleThresholdMinutes)}
						onChange={(event) =>
							patch({
				...
				<p className="muted wide">
					Bei Inaktivität über dieser Dauer schlägt TimeGlass vor, die
					Abwesenheit von der laufenden Session abzuziehen.
				</p>
				<button
					className="secondary-button wide"
					type="button"
					onClick={() =>
						void persistSettings("Stempel-Einstellungen gespeichert.")
					}
				>
					<Save size={16} aria-hidden="true" /> Stempeln speichern
				</button>
			</section>
			...
			<section className="glass-panel settings-form">
				...
				{remindersEnabled && diagnostics.notificationPermission !== "ja" && (
					<div className="inline-warning wide">
						Benachrichtigungen sind noch nicht erlaubt. TimeGlass fragt erst
						beim Speichern der Reminder nach.
					</div>
				)}
				<label className="switch-row wide">
					<input
						type="checkbox"
						checked={settings.reminderLongSessionEnabled}
						onChange={(event) =>
							patch({ reminderLongSessionEnabled: event.target.checked })
						}
					/>
				...
				<label>
					X Stunden
					<input
						value={formatMinutes(settings.reminderLongSessionMinutes)}
						onChange={(event) =>
							patch({
								reminderLongSessionMinutes:
									parseDurationToMinutes(event.target.value) ??
									settings.reminderLongSessionMinutes,
							})
						}
					/>
				</label>
				<label className="switch-row wide">
					<input
						type="checkbox"
						checked={settings.reminderClockOutEnabled}
						onChange={(event) =>
							patch({ reminderClockOutEnabled: event.target.checked })
						}
					/>
				...
				<label>
					...
					<input
						type="time"
						value={settings.reminderClockOutTime}
						onChange={(event) =>
							patch({ reminderClockOutTime: event.target.value })
						}
					/>
				</label>
				<label className="switch-row wide">
					<input
						type="checkbox"
						checked={settings.reminderNoTimeTodayEnabled}
						onChange={(event) =>
							patch({ reminderNoTimeTodayEnabled: event.target.checked })
						}
					/>
				...
				<label className="switch-row wide">
					<input
						type="checkbox"
						checked={settings.reminderTargetReachedEnabled}
						onChange={(event) =>
							patch({ reminderTargetReachedEnabled: event.target.checked })
						}
					/>
				...
				<label>
					...
					<input
						value={formatMinutes(settings.unusualSessionMinutes)}
						onChange={(event) =>
							patch({
								unusualSessionMinutes:
									parseDurationToMinutes(event.target.value) ??
									settings.unusualSessionMinutes,
							})
						}
					/>
				</label>
				<label className="switch-row wide">
					<input
						type="checkbox"
						checked={settings.notifyUnusualSession}
						onChange={(event) =>
							patch({ notifyUnusualSession: event.target.checked })
						}
					/>
				...
				<button
					className="secondary-button wide"
					type="button"
					onClick={() => void persistSettings("Reminder gespeichert.")}
				>
					<Save size={16} aria-hidden="true" /> Reminder speichern
				</button>
			</section>
			...
			<section className="glass-panel settings-form">
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							patch({ annualVacationDays: Number(event.target.value) })
						}
					/>
				</label>
				<label>
					...
					<input
						...
						onChange={(event) =>
							patch({ vacationCarryoverDays: Number(event.target.value) })
						}
					/>
				</label>
				<label>
					...
					<input
						type="number"
						value={settings.vacationYear}
						onChange={(event) =>
							patch({ vacationYear: Number(event.target.value) })
						}
					/>
				</label>
				<label>
					...
					<select
						value={settings.defaultPaidAbsenceBehavior}
						onChange={(event) =>
							patch({
								defaultPaidAbsenceBehavior: event.target
									.value as Settings["defaultPaidAbsenceBehavior"],
							})
						}
					>
						<option value="target_zero">Sollzeit 0 anzeigen</option>
						<option value="counts_as_target">
							Als Sollzeit erfüllt markieren
						</option>
					</select>
				</label>
				<label>
					...
					<select
						...
						onChange={(event) =>
							patch({ holidayRegion: event.target.value as GermanRegion })
						}
					>
				...
				<p className="muted wide">
					Importiert gesetzliche Feiertage des gewählten Bundeslands für das
					Urlaubsjahr als ganztägige Feiertags-Einträge (bereits vorhandene
					werden übersprungen).
				</p>
				<div className="button-row wide">
					...
					>
						<CalendarCheck size={16} aria-hidden="true" /> Feiertage{" "}
						{settings.vacationYear || new Date().getFullYear()} importieren
					</button>
					<button
						className="secondary-button"
						type="button"
						onClick={() =>
							void persistSettings("Urlaubseinstellungen gespeichert.")
						}
					>
						<Save size={16} aria-hidden="true" /> Urlaub speichern
					</button>
				</div>
			</section>
			...
			<section className="glass-panel settings-form">
				...
				<p className="muted wide">
					Regelmäßige Backups sind empfohlen. Es gibt keine Cloud-Sicherung.
				</p>
				<label className="switch-row wide">
					<input
						type="checkbox"
						...
						onChange={(event) =>
							patch({ autoBackupEnabled: event.target.checked })
						}
					/>
				...
				<div className="button-row wide">
					<button
						...
						type="button"
						onClick={() =>
							void persistSettings("Backup-Einstellungen gespeichert.")
						}
					>
						<Save size={16} aria-hidden="true" /> Backup-Optionen speichern
					</button>
					<button
						className="secondary-button"
						type="button"
					...
					>
						<HardDriveDownload size={16} aria-hidden="true" /> DB-Backup jetzt
					</button>
					<button
						className="secondary-button"
						type="button"
					...
					>
						<FolderOpen size={16} aria-hidden="true" /> Backup-Ordner öffnen
					</button>
				</div>
				<div className="button-row wide">
					<button
					...
					>
						<Download size={16} aria-hidden="true" /> CSV exportieren
					</button>
					<button
						className="secondary-button"
						type="button"
					...
					>
						<FileJson size={16} aria-hidden="true" /> JSON exportieren
					</button>
					<button
						className="secondary-button"
						type="button"
					...
					>
						<Upload size={16} aria-hidden="true" /> JSON importieren
					</button>
					<input
						ref={fileInput}
						hidden
			...
			<section className="glass-panel settings-form">
				...
				<DiagnosticsLine
					label="time_entries"
					value={String(diagnostics.timeEntryCount)}
				/>
				<DiagnosticsLine
					label="leave_entries"
					value={String(diagnostics.leaveEntryCount)}
				/>
				<DiagnosticsLine
					label="Aktive Session"
					value={data.entries.some((entry) => !entry.end_time) ? "ja" : "nein"}
				/>
				<DiagnosticsLine
					label="Letzter Export"
					value={data.settings.lastExportAt || "-"}
				/>
				<DiagnosticsLine
					label="Autostart aktiv"
					value={diagnostics.autostartActive ? "ja" : "nein"}
				/>
				<DiagnosticsLine
					label="Benachrichtigungen erlaubt"
					value={diagnostics.notificationPermission}
				/>
				<DiagnosticsLine
					label="Low-RAM-Modus"
					value={settings.lowRamMode ? "ja" : "nein"}
				/>
			</section>
		</div>
	);
