	...
	return (
		<div className="page-stack">
			...
			<div className="card-grid">
				<StatCard
					label="Erstes Einstempeln"
					value={formatClock(data.today.firstStart)}
				/>
				<StatCard
					label="Letztes Ausstempeln"
					value={formatClock(data.today.lastEnd)}
				/>
				<StatCard
					label="Brutto"
					value={formatMinutes(data.today.grossMinutes)}
				/>
				<StatCard
					label="Pause"
					value={formatMinutes(data.today.breakMinutes)}
				/>
				<StatCard label="Netto" value={formatMinutes(data.today.netMinutes)} />
				{noDailyTarget ? (
					<StatCard
						label="Soll"
						value="Kein Tages-Soll"
						detail="in diesem Arbeitsmodell"
						tone="neutral"
					/>
				) : (
					<StatCard
						label="Soll"
						value={formatMinutesOrDash(data.today.targetMinutes)}
					/>
				)}
				{showDailyDelta && !noDailyTarget ? (
					<StatCard
						...
						value={
							data.today.differenceMinutes != null
								? formatMinutes(data.today.differenceMinutes, true)
								: "—"
						}
						tone={
							data.today.differenceMinutes != null &&
					...
					<StatCard
						label="Differenz"
						value="nicht aktiv"
						detail="Kein Tages-Soll in diesem Arbeitsmodell"
						tone="neutral"
					/>
				)}
				<StatCard
					label="Tagesart"
					value={dayTypeLabels[data.today.dayType] ?? data.today.dayType}
					detail={data.today.note ?? undefined}
				/>
			</div>
				...
				<div className="card-grid three">
					{!noDailyTarget && (
						<StatCard
							...
							detail={
								exitOptions.dailyTargetReached ? "erreicht" : "noch offen"
							}
					...
					{hasAccountValue && (
						<StatCard
							...
							detail={
								exitOptions.zeroBalanceReached
									? "Konto bleibt mindestens 0"
									: "bis Gleitzeitkonto 0"
							}
						...
						<StatCard
							...
							detail={
								...
								exitOptions.coveredByFlexMinutes > 0
									? `Gleitzeit deckt ${formatMinutes(exitOptions.coveredByFlexMinutes)} ab`
									: undefined
							}
							tone={
								(exitOptions.currentBalanceIncludingTodayMinutes ?? 0) >= 0
									? "positive"
									: "negative"
							}
						/>
