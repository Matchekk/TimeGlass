...
function calendarButtonLabel(
	...
	return [
		...
		showDelta && item && item.differenceMinutes != null
...
export function MonthPage({
	...
	const monthSummaries = monthIsVisible
		? data.year.filter((day) => {
		...
		<div className="page-stack">
			<div className="section-heading">
				...
				<div className="month-nav" aria-label="Monat wechseln">
					...
					<button
						className="icon-button"
						type="button"
						aria-label="Monatsbericht drucken"
						title="Bericht drucken"
						disabled={!monthIsVisible}
			...
			<section className="calendar-grid glass-panel">
				...
				{gridDates.map((dateKey) => {
					...
					const dayHasDelta =
						showDailyDelta && item && item.differenceMinutes != null;
					return (
						<button
						...
						>
							...
							{dayHasDelta ? (
								...
								>
									...
									<small
										className={
											item!.differenceMinutes! >= 0
												? "delta-word positive-badge"
												: "delta-word negative-badge"
										}
									>
							...
							{leaveLabel && (
								<small className="state-badge leave-badge">{leaveLabel}</small>
							)}
						</button>
					);
				})}
