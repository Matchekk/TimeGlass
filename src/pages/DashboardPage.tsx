...
export function DashboardPage({
	...
	const [desiredPlus, setDesiredPlus] = useState(() =>
		formatMinutes(data.settings.desiredBalanceMinutes),
	);
	useEffect(() => {
	...
	const week = useMemo(
		...
		[data.week, data.settings],
	);
	const month = useMemo(
		() =>
			summarizePeriod(data.month, { settings: data.settings, kind: "month" }),
		[data.month, data.settings],
	);
	const year = useMemo(
		() => summarizePeriod(data.year, { settings: data.settings, kind: "year" }),
		[data.year, data.settings],
	);
	const totalTracked = useMemo(
		() => calculateTotalTrackedTime(data.allDays),
		[data.allDays],
	);
	const desiredPlusMinutes =
		parseDurationToMinutes(desiredPlus) ?? data.settings.desiredBalanceMinutes;
		...
		try {
			...
			setError(
				err instanceof Error
					? err.message
					: "Konto-Plus konnte nicht gespeichert werden.",
			);
		}
	}
	// Bei aktiver Session schliesst calculateFlexBalance den heutigen Tag bereits
	// aus dem Konto aus -> der Kontostand entspricht dann schon dem Stand "vor heute".
	// Ohne aktive Session ist heute im Konto enthalten und wird herausgerechnet.
	const balanceBeforeToday =
		data.flexBalanceMinutes == null
			? null
			: activeEntry
	...
	const exitOptions = useMemo(
		...
		[
			activeEntry,
			balanceBeforeToday,
			data.today.netMinutes,
			data.today.targetMinutes,
			desiredPlusMinutes,
		],
	);
	const suspiciousDays = useMemo(
		() =>
			findSuspiciousDays(
				data.allDays.length ? data.allDays : [data.today],
				data.entries,
				data.settings,
			).slice(0, 4),
		...
		<div className="page-stack">
			<header className="hero glass-panel">
				<div>
					<span className="eyebrow">
						{activeEntry ? "Eingestempelt" : "Ausgestempelt"}
					</span>
					<h1>
						{activeEntry
							? `Heute aktiv seit ${formatClock(activeEntry.start_time)}`
							: "Bereit für den Tag"}
					</h1>
					<p>
						{activeEntry
							? `Aktuelle Session: ${formatMinutes(data.today.grossMinutes)} brutto mit laufender Zeit`
							: "Private Übersicht, offline und lokal gespeichert."}
					</p>
					{isLongActiveSession(
						activeEntry,
						data.settings.unusualSessionMinutes,
					) && (
						<div className="inline-warning">
							Die aktive Session läuft ungewöhnlich lang. Keine Korrektur wurde
			...
			<div className="card-grid dashboard-grid">
				<StatCard
					...
					detail={
						activeEntry
							? formatClock(activeEntry.start_time)
				...
				<StatCard
					...
					detail={
						showDailyDelta ? (
							<DiffValue minutes={data.today.differenceMinutes} />
						) : (
							"Nettozeit"
						)
					}
					tone={
						showDailyDelta && data.today.differenceMinutes != null
							? data.today.differenceMinutes >= 0
								? "positive"
								: "negative"
							: "neutral"
					}
				...
				<StatCard
					...
					tone={
						week.differenceMinutes != null
							? week.differenceMinutes >= 0
								? "positive"
								: "negative"
							: "neutral"
					}
				...
				<StatCard
					...
					tone={
						month.differenceMinutes != null
							? month.differenceMinutes >= 0
								? "positive"
								: "negative"
							: "neutral"
					}
				...
				<StatCard
					...
					tone={
						year.differenceMinutes != null
							? year.differenceMinutes >= 0
								? "positive"
								: "negative"
							: "neutral"
					}
				...
				{noTargetMode ? (
					...
					<StatCard
						...
						detail={
							activeEntry
								? "Stand vor heute · aktualisiert beim Ausstempeln"
			...
			{variableWeeklyMode && week.targetMinutes != null && (
				<section
					className="glass-panel leave-calculator"
					aria-label="Wochenfortschritt"
				>
					<div>
						<span className="eyebrow">Wochenfortschritt</span>
						<h2>
							{weekRemaining === 0 ? "Wochenziel erreicht" : "Wochenziel offen"}
						</h2>
					</div>
					<div className="leave-calculator-grid">
						...
						<div
							className={`glass-card leave-mini-card ${weekRemaining === 0 ? "positive" : ""}`}
						>
							<span>Wochenrest</span>
							<strong>
								{weekRemaining != null ? formatMinutes(weekRemaining) : "—"}
							</strong>
			...
			{showExitPanel && (
				<section
					className="glass-panel leave-calculator"
					aria-label="Wann kann ich gehen"
				>
					<div>
						...
						{hasAccountValue &&
							exitOptions.coveredByFlexMinutes != null &&
							exitOptions.coveredByFlexMinutes > 0 && (
								<p className="muted">
									Gleitzeit deckt heute bis zu{" "}
									{formatMinutes(exitOptions.coveredByFlexMinutes)} ab.
								</p>
							)}
					</div>
					<div className="leave-calculator-grid">
						<div
							className={`glass-card leave-mini-card ${exitOptions.dailyTargetReached ? "positive" : ""}`}
						>
							<span>Rest bis Tages-Soll</span>
							<strong>
								{exitOptions.remainingToDailyTargetMinutes != null
									? formatMinutes(exitOptions.remainingToDailyTargetMinutes)
									: "—"}
							</strong>
						...
						{hasAccountValue ? (
							<div
								className={`glass-card leave-mini-card ${exitOptions.zeroBalanceReached ? "positive" : ""}`}
							>
								<span>Rest bis Konto 0</span>
								<strong>
									{formatMinutes(
										exitOptions.remainingToZeroBalanceMinutes ?? 0,
									)}
								</strong>
								<small>
									Feierabend{" "}
									{exitOptions.exitAtZeroBalance
										? formatClock(exitOptions.exitAtZeroBalance.toISOString())
										: "—"}
								</small>
							</div>
						) : (
					...
					{hasAccountValue && (
						<p className="muted" aria-live="polite">
							Aktuelles Gleitzeitkonto inklusive heute:{" "}
							{formatMinutes(
								exitOptions.currentBalanceIncludingTodayMinutes ?? 0,
								true,
							)}
						</p>
					)}
			...
			{activeEntry && !showExitPanel && !variableWeeklyMode && (
				<section
					className="glass-panel leave-calculator"
					aria-label="Hinweis Tagesziel"
				>
					<div>
						<span className="eyebrow">Wann kann ich gehen?</span>
						<h2>Für dieses Arbeitsmodell ist kein Sollziel hinterlegt.</h2>
						<p className="muted">
							TimeGlass erfasst in diesem Modell nur die gearbeitete Zeit. Über-
							und Unterstunden werden nicht berechnet.
						</p>
					</div>
				</section>
			)}
			...
			<section
				className="glass-panel quick-actions"
